...
bool ALCBossLich::RequestAttack(float TargetDistance)
{
    ...
    // Arcane Bolt
    if (bHasTarget && ArcaneBoltClass && ArcaneBoltRange > 0.f)
    {
        float Cooldown = bIsBerserk
            ? ArcaneBoltCooldown * BerserkCooldownFactor
            : ArcaneBoltCooldown;

        if (TargetDistance <= ArcaneBoltRange
            && Now - LastArcaneBoltTime >= Cooldown)
        {
            Entries.Add({
                3.f,
                [this, Now, Target]() {
    ...
    // Soul Bind
    if (bHasTarget && SoulBindRange > 0.f)
    {
        float Cooldown = bIsBerserk
            ? SoulBindCooldown * BerserkCooldownFactor
            : SoulBindCooldown;

        if (TargetDistance <= SoulBindRange
            && Now - LastSoulBindTime >= Cooldown)
        {
            Entries.Add({
                2.f,
                [this, Now, Target]() {
